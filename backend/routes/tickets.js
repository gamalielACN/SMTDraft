...
const { tickets, projects } = require('../data/mockData');
  ...
  try {
    if (useMockData) {
      ...
      if (req.body.type === 'seat_allocation' && req.body.formData) {
        ...
        // Validate that we have the right number of employee emails
        if (employeeEmails && employeeEmails.length !== parseInt(seatCount)) {
          return res.status(400).json({ 
            message: `Number of employee emails (${employeeEmails.length}) must match seat count (${seatCount})` 
          });
        }
