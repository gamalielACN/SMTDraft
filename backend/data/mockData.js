...
const createSeatAssignmentsFromRequests = () => {
  ...
  seatRequests.forEach(request => {
    if (request.status === 'approved' && request.seatIds && request.employeeEmails) {
      request.seatIds.forEach((seatId, index) => {
        const employeeEmail = request.employeeEmails[index];
        if (employeeEmail) {
          assignments.push({
            id: String(assignmentId++),
            seatId: seatId,
            facilityId: seatInventory.find(s => s.id === seatId)?.facilityId || '1',
