...
import { Search, MapPin, Building, Download, Calendar, AlertCircle, Filter, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function SeatsPage({ userRole }: SeatsPageProps) {
  ...
  const getSeatAssignment = (seatId: string) => {
    return seatAssignments.find(sa => sa.seatId === seatId && sa.isActive);
  };
  ...
  const getSeatStatus = (seatId: string) => {
    ...
    if (today > endDate) {
      return 'available'; // Past assignment
              ...
              <Table>
                ...
                <TableBody>
                  {seats.map((seat) => {
                    ...
                    return (
                      <TableRow key={seat.id} className="hover:bg-accenture-50/50">
