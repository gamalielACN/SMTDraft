...
export default function NewTicketPage() {
  ...
  // Update employee emails array when seat count changes
  useEffect(() => {
    if (formData.type === 'seat_allocation' && formData.seatCount) {
      const seatCount = parseInt(formData.seatCount) || 0;
      const newEmails = Array(seatCount).fill('').map((_, i) => employeeEmails[i] || '');
      setEmployeeEmails(newEmails);
      setFormData(prev => ({ ...prev, employeeEmails: newEmails }));
    }
  }, [formData.seatCount, formData.type]);
  ...
  }, [formData.projectId, formData.type, activeProjects]);
      ...
      switch (formData.type) {
        ...
        case 'seat_allocation':
          ticketFormData = {
            ...
            employeeEmails: formData.employeeEmails.filter(email => email.trim() !== ''),
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                  {employeeEmails.map((email, index) => (
                    <div key={index}>
                      <Label htmlFor={`employee-${index}`} className="text-sm">
                      ...
                      <Input
                        ...
                        placeholder={`employee${index + 1}@accenture.com`}
