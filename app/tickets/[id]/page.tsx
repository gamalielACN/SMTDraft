...
export default function TicketDetailPage() {
  ...
  const getAvailableSeatsForFacility = (facilityId: string, currentEmployeeEmail?: string) => {
    ...
    if (!facility || facility.metroCity !== project.metroCity) {
      return []; // Only show seats from facilities in same metro city as project
    }
  ...
  useEffect(() => {
    if (ticket?.type === 'seat_allocation' && user?.role === 'business_ops') {
      fetchSeatData();
    }
  }, [ticket, user?.role]);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (ticket.type === 'seat_allocation') {
        // Validate that all employees have seat assignments
        const employeeEmails = ticket.formData.employeeEmails || [];
        const missingAssignments = employeeEmails.filter((email: string) => !ticketSeatAssignments?.[email]);
        
        if (missingAssignments.length > 0) {
          setError(`Please assign seats for all employees: ${missingAssignments.join(', ')}`);
          return;
        }
        
        // Create seat requests and assignments for each employee
        const seatRequestPromises = employeeEmails.map(async (email: string, index: number) => {
        ...
        try {
          ...
          toast.success('Seat assignments created successfully!', {
            description: `${employeeEmails.length} seat request(s) have been created and approved.`
          });
        } catch (seatError) {
  ...
  const renderSeatAssignmentSection = () => {
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="text-sm text-muted-foreground mb-4">
            Assign specific seats to each employee before approving the request.
          </div>
            ...
            {employeeEmails.map((email: string, index: number) => (
              <div key={index} className="border rounded-lg p-3 space-y-3">
                ...
                <div>
                  ...
                  <Select 
                    value={ticketSeatAssignments?.[email] || ''} 
                  ...
                  >
                    ...
                    <SelectContent>
                      {facilities.map((facility) => {
                        ...
                        const availableSeatsInFacility = getAvailableSeatsForFacility(facility.id, email);
                        
                        if (availableSeatsInFacility.length === 0) return null;
                        ...
                        return (
                          <div key={facility.id}>
                            ...
                            {availableSeatsInFacility.map((seat) => (
                      ...
                      {facilities && facilities.filter(f => {
                        const project = getProjectInfo(ticket.projectId);
                        return project && f.metroCity === project.metroCity;
                      }).length === 0 && (
                        <div className="px-2 py-1.5 text-sm text-muted-foreground">
                          No facilities available in {getProjectInfo(ticket.projectId)?.metroCity}
                        </div>
                      )}
                    </SelectContent>
                  </Select>
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  {Object.keys(ticketSeatAssignments || {}).length} of {employeeEmails.length} employees assigned
                </p>
                {(() => {
                  const project = getProjectInfo(ticket.projectId);
                  const facilitiesInCity = facilities ? facilities.filter(f => project && f.metroCity === project.metroCity) : [];
                  const totalAvailableSeats = facilitiesInCity.reduce((total, facility) => {
                    return total + getAvailableSeatsForFacility(facility.id).length;
                  }, 0);
                  
                  if (totalAvailableSeats === 0 && project) {
                    return (
                      <p className="text-sm text-orange-700 mt-1">
                        ⚠️ No available seats in {project.metroCity}
                      </p>
                    );
                  }
                  return null;
                      ...
                      {ticket.type === 'seat_allocation' ? (
                        <Button
                          ...
                          disabled={
                            !comment.trim() || 
                            isUpdating || 
                            (ticket.formData.employeeEmails && 
                             !ticket.formData.employeeEmails.every((email: string) => ticketSeatAssignments?.[email]))
                          }
                          className="flex-1"
                        >
