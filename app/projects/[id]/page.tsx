...
import { ArrowLeft, Building, Calendar, Users, MapPin, Mail, DollarSign, FileText, Wheat as Seat, AlertCircle, CheckCircle, Clock, User } from 'lucide-react';
  ...
  const fetchProjectData = async () => {
    try {
      ...
      const [projectData, seatRequestsData, invoicesData, employeesData, ticketsData] = await Promise.all([
        ...
        apiClient.getProjectEmployees(projectId),
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            {/* Project Team - Only show if employees exist */}
            {employees.length > 0 && (
              <Card className="card-hover">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <User className="h-5 w-5 text-accenture-500" />
                    Project Team ({employees.length}) - From Seat Allocations
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {employees.map((employee) => (
                        <TableRow key={employee.id} className="hover:bg-accenture-50/50">
            ...
            <Card className="card-hover">
              ...
              <CardContent>
                <div className="space-y-3">
                  {project.wbsEntries.map((wbs) => (
                    <div key={wbs.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-accenture-50/50 transition-colors">
                      <div>
                ...
                {invoices.length > 0 ? (
                  <div className="space-y-4">
                    {invoices.map((invoice) => (
                      <div key={invoice.id} className="border rounded-lg p-4 hover:bg-accenture-50/50 transition-colors">
